  ...
  if (type == "EMMA") {
    ...
    for (i in 1:length(flist)){
      ...
      if (n == 1) {
        ...
        for (ntraits in 0:(n-1)) {
          nametemp <- get(str_sub(flist[i], 1, -5))[!duplicated(get(str_sub(flist[i], 1, -5)))[,1],1]
          plot_file[, ncol(plot_file) + ntraits] <- get(str_sub(flist[i], 1, -5))$'p'[(ntraits*nrow(hmp)+1):(nrow(hmp)*(ntraits+1))]
          names(plot_file)[ncol(plot_file)] = nametemp[ntraits + 1]
        }
      }
    }
  }
...
sigsnp <- function(plot_file){
    ...
    names(sigsnp_hmp)[6:10] <- c("Allele1", "Allele2", "Allele3", "AlleleT", names(sigsnp_temp)[4])
    return(sigsnp_hmp)
}
...
MTA_range <- function(sigsnp_hmp, LDG){
    ...
    if (nrow(snp_chr) == 1) {
      ...
      for (n in 2:nrow(snp_chr)) {
        ...
        if (pos_dis < LDG*1000000) {
          assign(paste0(traitname, chr[i], "MTA_", index1), snp_chr[(n-index2):n,])
          index2 <- index2 + 1
        }else{
          index1 <- index1 + 1 
          assign(paste0(traitname, chr[i], "MTA_", index1), snp_chr[index2:n,])
          index2 <- 1
        }
      }
    }
  ...
  for (i in 1:length(MTA_1_trait)) {
    ...
    MTA <- filter(hmp, chrom == snp[1,3], pos > snp[1,4] - LDG*10e5, pos <  snp[n,4] + LDG*10e5)
